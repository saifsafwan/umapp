        ...
        <div class="md:w-1/4 md:mx-auto">
            ...
            <div class="flex flex-col break-words bg-white border-2 rounded shadow-md">
                ...
                <div class="font-semibold bg-gray-200 text-gray-700 py-3 px-6 mb-0">
                    Edit Budget's Item : {{$budget->title}} - {{$item->name}}
                </div>
                <form action="{{route('item.update', $item->id)}}" method="POST">
                    ...
                    <div class="w-3/4 p-6 mx-auto">
                        <label class="block">
                            ...
                            <input type="text" name="name" class="form-input mt-1 block w-full" value="{{$item->name}}">
                        </label>
                        <label for="item_type" class="block text-gray-700 text-sm font-semibold mb-2 my-4">
                            Item Type
                        </label>
                        <div class="flex">
                        ...
                        <label class="block mt-4">
                            ...
                            <div class="flex">
                                ...
                                <input type="number" step="0.10" name="unit_price" class="form-input block w-full" value="{{$item->unit_price}}">  
                            </div>
                        </label>
                        <label class="block mt-4">
                            <span class="text-gray-700 block mb-2 font-semibold">Quantity</span>
                            <input type="number" name="quantity" class="form-input mt-1 block w-full" value="{{$item->quantity}}">
                        </label>
                        <label class="block mt-4">
                            <span class="text-gray-700 block mb-2 font-semibold">Unit of measurement</span>
                            <input type="text" name="uom" class="form-input mt-1 block w-full" value="{{$item->uom}}">
                        </label>
                        <input type="hidden" name="budget_id" value="{{$budget->id}}">
                        <button class="mt-4 p-4 rounded-md bg-accountant text-white" type="submit">Save Changes</button>
